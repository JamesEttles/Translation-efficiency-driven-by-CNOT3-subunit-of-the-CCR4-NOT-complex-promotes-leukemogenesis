...
#do
#bbmap.sh in=$fastq_dir/${filename}_.fastq out=$SAM_dir/${filename}_rRNA.sam ref=$rRNA_fasta outm=$fastq_dir/${filename}_rRNA.fastq outu=$fastq_dir/${filename}_non_rRNA.fastq ambiguous=best nodisk threads=16 2> #$log_dir/${filename}_rRNA_log.txt
#done

#Align to protein coding transcriptomes
for filename in $Totals_filenames
do
rsem-calculate-expression --paired-end $fastq_dir/${filename}_R1.fastq $fastq_dir/${filename}_R2.fastq --strandedness forward --bowtie2 $rsem_index $rsem_dir/${filename} &
done
wait
